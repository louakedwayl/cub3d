...
/*   Updated: 2025/02/04 13:45:15 by ajosse           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	free_all_and_exit(t_data *data, int exitcode, char *optional_msg)
{
	...
	if (data->mlx_data != NULL)
	{
		...
		if (data->parsing_data->img_north.img)
			mlx_destroy_image(data->mlx_data->mlx, data->parsing_data->img_north.img);
		if (data->parsing_data->img_south.img)
			mlx_destroy_image(data->mlx_data->mlx, data->parsing_data->img_south.img);
		if (data->parsing_data->img_east.img)
			mlx_destroy_image(data->mlx_data->mlx, data->parsing_data->img_east.img);
		if (data->parsing_data->img_west.img)
			mlx_destroy_image(data->mlx_data->mlx, data->parsing_data->img_west.img);
